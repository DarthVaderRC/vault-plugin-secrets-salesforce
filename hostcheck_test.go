...
func TestValidateTokenHost(t *testing.T) {
	cases := []struct {
		...
		{"loopback http rejected without opt-in", "http://127.0.0.1:8080/services/oauth2/token", false, true},
		{"localhost rejected without opt-in", "http://localhost:9000/x", false, true},
		{"loopback http allowed with opt-in", "http://127.0.0.1:8080/services/oauth2/token", true, false},
		{"localhost allowed with opt-in", "http://localhost:9000/x", true, false},
